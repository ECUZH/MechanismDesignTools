...
import ch.uzh.ifi.Mechanisms.ProbabilisticReverseCAFactory;

public class GenericProbabilisticReverseAuctionBNE {
	...
	public static void main(String[] args) 
	{
		...
		String paymentRule	   = args[9 + offset];						if( !(paymentRule.equals("Exp-VCG") || paymentRule.equals("Exp-Core")) ) throw new RuntimeException("Wrong payment rule specified: " + paymentRule);
		...
		String plannerType     = args[10 + offset];						if( !(plannerType.equals("GeneralProbabilistic")) ) throw new RuntimeException("Wrong planner type specified:" + plannerType );
		String utilityEngine   = args[11 + offset];						if( !(utilityEngine.equals("MT") || utilityEngine.equals("MPI") ) ) throw new RuntimeException("Wrong utility computation engine specified: " + utilityEngine);
		String optimizationEngine=args[12+ offset];						if( !(optimizationEngine.equals("stochastic") || optimizationEngine.equals("deterministic") ) ) throw new RuntimeException("Wrong local optimization engine specified: " + optimizationEngine);
		int loadState 		   = Integer.parseInt(args[13 + offset]);	if( loadState != 0 && loadState != 1 )  throw new RuntimeException("Wrong load state");
		...
		if( plannerType.equals("GeneralProbabilistic"))
		{
			plannerFactory = new GeneralProbabilisticPlannerFactory(numberOfBuyers, numberOfSellers, bids, isInjectable, numberOfPlans, minNumberOfSellers, maxNumberOfSellers);
		}
		else
		{
		...
		ProbabilisticReverseCA market = new ProbabilisticReverseCA(numberOfBuyers, plannerFactory.producePlanner().generatePlans());
		...
		{
			throw new RuntimeException("MPI temp. not supported");
			//MPI.Init(args);
		...
		IMechanismFactory[] rcaFactory = {new ProbabilisticReverseCAFactory(numberOfBuyers, numberOfSellers, plannerFactory, paymentRule)};
		...
		//logger.info(paymentRule + " + " + "BNESolver(market, bids, "+settings.getNumberOfBins()+", "+
